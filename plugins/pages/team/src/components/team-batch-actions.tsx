...
interface TeamBatchActionsProps {
  selectedCount: number;
  onClear?: () => void;
  onDelete?: () => void;
}

export function TeamBatchActions({ selectedCount, onClear, onDelete }: TeamBatchActionsProps) {
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-popover text-popover-foreground px-3 py-2 flex items-center gap-3 shadow-2xl border border-border z-[100] rounded-sm">
      ...
      <div className="flex gap-1 flex-nowrap">
        ...
        >
          <i className="fa-solid fa-trash-can text-[10px]"></i> {t('batchActions.remove')}
        </button>
      </div>
      <button
        onClick={onClear}
