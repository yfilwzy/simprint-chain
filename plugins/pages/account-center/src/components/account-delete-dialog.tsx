  ...
  return (
    <FormattedDialog
      ...
      header={{
        ...
        description: t('dialog.delete.description', { account: account.account }),
      ...
      <div className="bg-muted/50 rounded-md p-3 border border-border/50">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-foreground">{account.account}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{account.platformName}</p>
            {account.environmentsCount > 0 && (
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                {t('dialog.delete.linkedEnvironments', {
                  count: account.environmentsCount,
                  defaultValue: `已关联 ${account.environmentsCount} 个环境`,
                })}
              </p>
            )}
          </div>
